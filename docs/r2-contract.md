...
#### 증적의 성격과 한계 (R2-R10-L1)

위 A/B 블록에서 **관찰**과 **추론**을 나눠 읽어야 한다.

- **관찰(재현 가능한 사실):** 라운드 9 의 프로덕션 변경은 `api_index.py` 의 인덱스
  **로드 시점** `source` 검증뿐이다. `cmd_fetch` 는 요청을 내보내기 전에 인덱스를
  로드하므로(engine.py `api_index_mod.load_cached`) 이 검증은 fetch 의 로드 단계에서
  인덱스를 거부할 수 있으나, **로드 이후의 네트워크 실행 경로**(요청 발신·구제·티어
  폴백)에는 관여하지 않는다. 이 사실은 코드로 확인되며 실행 환경과 무관하게 참이다 —
  A/B 의 두 실행이 동일 수치(33/36)를 낸 것이 그 방증이다.
- **추론(확정 아님):** 떨어진 3 건이 "DataDome 의 rate 기반 챌린지"라는 **원인 단정**은
  관찰된 사실(`by_reason: waf_challenge`, datadome 6→3)로부터의 **추정**이며, 본문의
  "보인다" 가 그 불확실성을 표시한다. 우리는 챌린지를 풀지 않으므로(NG 경계) 원인이
  무엇이든 이 결과는 정상 동작이라는 결론은 원인 단정에 의존하지 않는다.

**보존 공백을 명시한다.** 위 A/B 의 원출력(두 실행의 `by_vendor`·`by_reason`·`rate`)은
그대로 옮겨 적었으나 **저장소에 산출물로 커밋하지 않았다.** 배터리는 실사이트를 때리는
스위트라 라이브 벤더 상태에 좌우되고, 같은 명령을 지금 다시 돌려도 동일 수치가 나온다는
보장이 없다 — 그래서 이 A/B 는 **특정 시점의 라이브 측정**이며 동결 산출물로 재현할 수
없다. 재현이 필요하면 `bench --battery` 를 다시 돌려 그 시점의 라이브 상태를 새로
측정해야 한다. `bench/history.jsonl`(append-only)이 실행 이력의 항구 증적이고, 위
블록의 수치는 그 시점 실행의 사람이 읽는 요약이다.
