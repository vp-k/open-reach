...
- **AC-B-012-6**: 대상 호스트의 robots.txt 가 **정직한 접근을 허용하는 엔드포인트만** 등재한다. `User-agent: *` 전면 `Disallow: /` 호스트는 등재하지 않는다 — 등재해도 런타임 robots 재검사(AC-B-010-13)가 차단하므로, 등재는 "지원" 표기의 거짓이 된다 (NG-8). **실측 확정(2026-09-02, `bench/evidence/r5-adapter-probe-2026-09-02.json`)**: `search.naver.com`·`www.reddit.com` 은 `*` 전면 Disallow → 미등재. Threads 는 공개 게시물 데이터가 초기 HTML 에 없는 로그아웃 셸이라 URL 템플릿으로 표현 불가 → 미등재. Mastodon 은 인스턴스 호스트 가변이라 `host` 정확 일치 인덱스로 표현 불가 → 미등재. R5 등재는 **`news.ycombinator.com`(HN Algolia items)·`bsky.app`(getPostThread)** 2건이다.
...
> 원래 동기였던 네이버 통합검색은 실측 결과 robots 전면 Disallow 로 접근 자체가 정책 차단이다(AC-B-012-6 실측 확정) — 검증기 완화로 해결될 문제가 아니었다. 이 US 는 **선언 메커니즘**(`search:` 섹션, Data Model SearchDeclaration)으로 유지한다: robots 가 허용하는 검색 엔드포인트가 선언되어 있으면 그 URL 의 결과 목록을 본문으로 인정한다.
...
- **AC-B-014-4**: 검색 선언도 `source`·`verified_at` 의무와 20 항목 합산 상한(AC-B-010-15)을 진다. 누락·초과는 로드 실패(종료 코드 3)다. robots 가 정직한 접근을 막는 검색 엔드포인트는 선언하지 않는다(AC-B-012-6 승계).
...
옵션: `--intent article|media|raw`, `--timeout <초>`, `--max-attempts <N>`, `--allow-browser`, `--api-index <경로>`
  ...
  `--allow-reader` 는 도입하지 않는다 — US-B-013 은 철회됐다(`r.jina.ai` robots 전면 Disallow, US-B-013 철회 노트 참조).
