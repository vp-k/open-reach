...
- **AC-B-010-11**: 넘겨받은 값은 **경로 세그먼트 1개**로만 치환된다. `/`·`\`·`%`·`:`·`?`·`#`가 포함되면 거부하고, 값 자체가 `.` 또는 `..`이면 거부한다 (`value_pattern`이 이미 막더라도 이중으로 검사한다). 점은 **문자로는 금지하지 않는다** — `1.0.219` 같은 버전 문자열이 정상 세그먼트이기 때문이며, 경로 탈출은 `.`·`..` 세그먼트와 `/`·`\`·`%` 로 막는다.
...
> `r.jina.ai`는 우리 대신 대상을 가져와 마크다운으로 반환한다. 본문이 제3자를 경유하므로 기본 비활성이며 opt-in으로만 켠다. 경계는 `docs/r5-contract.md` §4-②.

- **AC-B-013-1**: Jina Reader 경로는 **`--allow-reader` 플래그가 있을 때만** 시도한다. 플래그가 없으면 시도 자체가 없고 `attempts[]`에도 리더 시도가 남지 않는다.
- **AC-B-013-2**: 리더 경로는 **가장 마지막 폴백**이다(HTTP·Phase 0·임퍼소네이션·브라우저가 모두 실패한 뒤). 원문이 정상인 사이트를 제3자로 우회시키지 않는다.
- **AC-B-013-3**: 리더 URL을 조립하기 **전에** 원본 URL을 기존 SSRF 가드로 검사한다. 사설·루프백·링크로컬·메타데이터 대상 URL은 리더로 보내지 않는다 (NG-11). 리더 응답이 우리를 다른 호스트로 보낼 수 없다.
- **AC-B-013-4**: 리더가 반환한 마크다운도 **US-B-008 검증기를 통과**해야 `ok=true`다. 리더가 챌린지·동의 배너 페이지를 그대로 마크다운화하면 `validation_failed`다.
- **AC-B-013-5**: 성공 시 `attempts[]`에 `route="reader"`인 시도가 남고, 제3자 게이트웨이를 경유했음이 결과에 표기된다. 소비자가 직접 취득 본문과 구분할 수 있어야 한다.
- **AC-B-013-6**: 리더 경로는 정책 계층(`docs/policy-boundaries.md`)에 제3자 경유 성질과 opt-in 요건이 문서화된다.

### US-B-014 — 명시적 검색 URL 본문 인정 (R5)

<!-- provenance: user-fact -->

As a 리서처, I want to 내가 검색 결과 URL을 직접 줬을 때는 그 결과 목록을 본문으로 받길 원한다, so that 네이버 검색 같은 어댑터 결과가 "검색 페이지라서 실패"로 오분류되지 않는다. (J1, J4)

- **AC-B-014-1**: 입력 URL이 **플랫폼 검색 엔드포인트**(어댑터가 검색 URL로 선언한 패턴)면, 검색 결과 목록을 유효 본문으로 인정한다(AC-B-008-1 예외).
...
| 필드 | 타입 | 제약조건 | 설명 |
|------|------|----------|------|
| route | str | `policy`/`phase0`/`http`/`browser` | 시도 계층 |
...
옵션: `--intent article|media|raw`, `--timeout <초>`, `--max-attempts <N>`, `--allow-browser`, `--allow-reader`, `--api-index <경로>`

  `--allow-reader`(R5, US-B-013)는 Jina Reader(`r.jina.ai`) 제3자 폴백을 opt-in으로 켠다. 기본 비활성이며, 켜도 다른 모든 경로가 실패한 뒤 **마지막**에만 시도한다. 원본 URL은 리더로 보내기 전에 SSRF 가드를 다시 통과해야 한다(AC-B-013-3).
...
- `robots.txt`를 조회해 `Disallow` 대상이면 `policy_blocked`로 중단한다. robots 조회 자체가 실패하면 해당 호스트의 기본 정책(허용)을 따르되 사유를 관측에 남긴다.
...
| **R5** | **US-B-012**(Phase 0 공개 플랫폼 어댑터 확장 — Naver·Reddit·HN·Bluesky·Threads-public) · **US-B-013**(Jina Reader opt-in 폴백) · **US-B-014**(명시적 검색 URL 본문 인정, AC-B-008-1 예외) — 근거·경계는 `docs/r5-contract.md`. 종료 조건은 `rate_http_only` 무회귀 + 각 어댑터 인수 테스트 green |
...
| NG-5 | 대량 크롤링 | `fetch`는 단건만, `bench`는 배터리 파일만, 호스트 동시성 1 |
