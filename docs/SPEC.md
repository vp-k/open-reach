...
- **AC-B-003-4**: 사설 IP·루프백·링크로컬·CGNAT 대역 또는 `http`/`https` 외 스킴 URL은 네트워크 요청 없이 `policy_blocked`로 거부된다 (NG-11).
...
- **AC-B-008-1**: HTTP 200이지만 본문이 WAF 챌린지·동의 배너·검색 결과·오류 페이지면 `ok=false`이며 `failure_reason`은 `waf_challenge` 또는 `validation_failed`다.
...
- **SSRF 차단 (fail-closed)**: 스킴 allowlist(`http`/`https`) → DNS 해석 → 사설(RFC1918)·루프백·링크로컬(169.254/16)·CGNAT(100.64/10)·IPv6 ULA와 링크로컬 차단 → **리디렉션 매 홉 재검사**. `169.254.169.254` 등 클라우드 메타데이터 주소는 명시 차단 목록에 둔다. 판정 불가 시 **차단**한다.
