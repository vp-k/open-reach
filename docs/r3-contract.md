...
브라우저 티어 구현으로 사실이 바뀐 SPEC 두 줄은 **이 계약 승인 시** 함께 반영한다.
- §41 "Playwright / patchright는 미설치 — 브라우저 티어는 **지연 설치 대상**" → 사실 유지
  (지연 설치 계약은 그대로다). 문구 조정 불요.
- §216 `allow_browser` "**R1에서는** 항상 false로 강제" → R3에서 티어가 생겼으므로
  "R1·R2에서는 false 강제, R3부터 `--allow-browser`로 opt-in" 취지로 갱신 제안.

두 줄 모두 SPEC 동결 대상이므로, 반영은 사용자 승인 하 재동결로만 한다(세탁 금지).
